...
export async function HomeServices() {
  ...
  const offers: (IOffer & { liked: boolean; subscribed: boolean; tags: string[] })[] = (offersWithRelations ?? []).map((offer) => {
    ...
    const tagNames = offer.tags?.map((tagRel) => tagRel.Tag.name).filter((name): name is string => typeof name === "string" && name.trim() !== "") || [];
