DROP FUNCTION IF EXISTS count_subscribers;
...
BEGIN
    ...
    EXECUTE format(
        'SELECT COUNT(*) FROM %I WHERE %I = $1 AND subscribed = true',
        user_table,
        post_column
    )
    INTO new_count
    ...
    EXECUTE format(
        'UPDATE %I SET current_progress = $1 WHERE id = $2 RETURNING current_progress',
        target_table
    )
    INTO new_count
