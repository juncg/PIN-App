...
export function SidebarForumCard({ forum }: SidebarForumCardProps) {
	return (
		...
		>
			<Avatar className={`h-10 w-10 rounded-lg border border-hover ${!forum.profile_picture && "bg-lightgrey"}`}>
			...
			<div className="flex flex-col overflow-hidden">
				...
				<div className="flex items-center gap-1">
					<span className="text-xs text-lightgrey truncate">{forum.Business?.name}</span>
				</div>
			</div>
		</Link>
	);
