...
export function CommentCard({
	...
	const handleReplySubmit = async () => {
		...
		try {
			...
			// referenced_user_id is null for top-level replies (replying to the post itself)
			const referencedUserId = isTopLevel ? null : comment.user?.id || null;
			...
			const newReply: IComment = {
				...
				// extended fields
				user: currentUser,
				referencedUser: isTopLevel ? undefined : comment.user,
				replies: [],
				replyCount: 0,
			};
					...
					{comment.referencedUser && (
						<>
							...
							<span className="text-xs text-lightgrey">
								respondiendo a <span className="text-blue-400">@{comment.referencedUser.username}</span>
							</span>
						</>
					)}
