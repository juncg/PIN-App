...
import { Command, CommandGroup, CommandItem, CommandList } from "@/components/ui-custom/command";
					...
					{selected.map((option) => {
						return (
							<div
								key={option.value}
								className={cn(
									"animate-fadeIn bg-darkmode text-white hover:bg-hover relative inline-flex h-7 cursor-default items-center rounded-md border border-white pr-7 pl-2 text-xs font-medium transition-all disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 data-fixed:pr-2",
									badgeClassName
								)}
